...
*/
	...
	if(movingUpNode->parent != NULL){
		...
		if(movingUpNode->abbr > movingUpNode->parent->abbr){
			...
			if (newnode->abbr < tmp->abbr) {
				if (tmp->left == NULL) {
					...
					while(tmpParent != NULL){
						...
						if(getBalance(tmpParent) > 1){
							if(tmpParent->left != NULL && getBalance(tmpParent->left) < 0){
								tmpParent = rotateLeft(tmpParent->left);
								tmpParent = rotateRight(tmpParent->parent);
							}
							else{
								tmpParent = rotateRight(tmpParent);
							}
						}
						else if(getBalance(tmpParent) < -1){
							if(tmpParent->right != NULL && getBalance(tmpParent->right) > 0){ // left-right
								tmpParent = rotateRight(tmpParent->right);
								tmpParent = rotateLeft(tmpParent->parent);
							}
							else{
								tmpParent = rotateLeft(tmpParent);
							}
						}
